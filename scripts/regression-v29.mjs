...
assert(Math.max(...playerIndexes) < 168, 'player portraits should use canonical identity indexes only');
assert(Math.max(...coachIndexes) < 40, 'coach portraits should use canonical identity indexes only');
assert(new Set(playerIndexes).size >= 70, 'player portrait distribution is unexpectedly narrow');
assert(new Set(coachIndexes).size >= 22, 'coach portrait distribution is unexpectedly narrow');
...
assert(appSource.includes("const APP_VERSION = '0.28.4'"), 'app version marker is stale');
...
assert(cssSource.includes('v0.28.4 FACE EXPANSION PASS'), 'polish CSS patch is missing');
