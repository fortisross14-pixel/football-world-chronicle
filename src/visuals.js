...
const HISTORICAL_ICON_PORTRAITS = new Set([
  'pele',
  'maradona',
  'messi',
  'cristiano-ronaldo',
...
function historicalPortraitId(entity) {
  const id = entity?.realWorldId || entity?.historicalId || '';
  return HISTORICAL_ICON_PORTRAITS.has(id) ? id : '';
}
...
function portraitImage(entity, kind = 'player', nationalityName = '', size = 'md') {
  ...
  const historicalId = kind === 'player' ? historicalPortraitId(entity) : '';
  const source = historicalId
    ? `./assets/faces/historical/${historicalId}.webp`
    : `./assets/faces/${kind === 'coach' ? 'coaches' : 'players'}/${prefix}${String(idx).padStart(3,'0')}.webp`;
  const identityClass = historicalId ? ' historical-avatar' : '';
  const dataAttrs = historicalId
    ? `data-historical-portrait="${historicalId}"`
    : `data-avatar-pool="${idx}"`;
  const title = historicalId
    ? `${displayName} curated historical portrait`
    : `${kind === 'coach' ? 'Coach' : 'Player'} portrait #${idx + 1}`;
  return `<span class="anime-avatar ${kind}-avatar${rarityClass}${identityClass} ${sizeClass}" ${dataAttrs} title="${title.replace(/"/g,'&quot;')}"><span class="anime-face-fallback" aria-hidden="true">${fallback.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span><img data-face-image class="anime-face-image" src="${source}" alt="${alt.replace(/"/g,'&quot;')}" loading="lazy" decoding="async"></span>`;
}
...
export function getFacePoolPlan() {
  return JSON.parse(JSON.stringify({
    ...
    historical: {
      total: HISTORICAL_ICON_PORTRAITS.size,
      ids: [...HISTORICAL_ICON_PORTRAITS]
    }
  }));
}


export function getHistoricalPortraitPlan() {
  return {
    total: HISTORICAL_ICON_PORTRAITS.size,
    ids: [...HISTORICAL_ICON_PORTRAITS]
  };
