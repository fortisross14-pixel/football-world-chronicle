...
function playerLeagueContext(player) {
  ...
  if (!club) return '';
  const rows = currentLeaguePowerRows().filter((row)=>row.league.confederation===club.confederation);
  const rank = rows.findIndex((row)=>row.league.id===club.leagueId)+1;
  const league = LEAGUE_DEFINITIONS.find((row)=>row.id===club.leagueId);
  return rank ? `${league?.name || club.leagueId} #${rank} in ${club.confederation}` : '';
}
...
function offseasonTabs(active) {
  return `<nav class="competition-tabs"><a href="#/offseason/summary" class="${active==='summary'?'active':''}">Summary</a><a href="#/offseason/signings" class="${active==='signings'?'active':''}">Signings</a><a href="#/offseason/coaches" class="${active==='coaches'?'active':''}">Coach Market</a><a href="#/offseason/lifecycle" class="${active==='lifecycle'?'active':''}">Spawn & Retirements</a><a href="#/offseason/boardroom" class="${active==='boardroom'?'active':''}">Boardroom</a></nav>`;
}

function offseasonPage(tabRaw='summary') {
  const data = state.current.offseason;
  const tab = ['summary','signings','coaches','lifecycle','boardroom'].includes(tabRaw) ? tabRaw : 'summary';
  ...
  const ownerChanges = (data.ownerChanges||[]).slice(0,50);
